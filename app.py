...
Features:
- ✅ Fixed login flow with st.rerun()
- ✅ Fixed language toggle with st.rerun()
...
T = {
    'en': {
        'title': 'Axiom AI', 'subtitle': 'PROFESSIONAL ANALYTICS PLATFORM', 'login_title': 'Welcome to Axiom AI',
        'name': 'Full Name', 'email': 'Email', 'age': 'Age',
        'privacy': 'I agree that my data is processed in real-time RAM and never stored on any server.',
        'btn_login': 'Enter Platform', 'denied': 'Access Denied - Must be 14+', 'btn_back': 'Go Back',
        'dashboard': 'Dashboard', 'welcome': 'Welcome to Axiom AI, {}!',
        'input_label': 'Data Input', 'expert_label': 'Select Your Expert:',
        'expert_financial': '💰 Financial Expert', 'expert_marketing': '📈 Marketing Expert', 'expert_operations': '⚙️ Operations Expert',
        'placeholder': 'Enter data...\n\nExample:\nSales: $50,000\nExpenses: $30,000',
        'btn_analyze': 'Analyze Data', 'results': 'AI Insights', 'btn_copy': 'Copy', 'btn_pdf': 'Export PDF', 'btn_new': 'New Analysis',
        'privacy_notice': '🔐 All data processed in RAM and wiped instantly.', 'support': 'Need help? axiomai873@gmail.com',
        'btn_logout': 'Logout', 'lang_btn': 'العربية', 'api_key': 'Groq API Key', 'api_help': 'Get key from console.groq.com'
    },
    'ar': {
        'title': 'Axiom AI', 'subtitle': 'منصة التحليل الاحترافية', 'login_title': 'مرحباً بك',
        'name': 'الاسم الكامل', 'email': 'البريد الإلكتروني', 'age': 'العمر',
        'privacy': 'أوافق على أن بياناتي تُعالج في RAM ولا تُخزن على أي خادم.',
        'btn_login': 'دخول المنصة', 'denied': 'رفض الوصول - يجب أن يكون العمر 14+', 'btn_back': 'رجوع',
        'dashboard': 'لوحة التحكم', 'welcome': 'مرحباً بك، {}!',
        'input_label': 'إدخال البيانات', 'expert_label': 'اختر الخبير:',
        'expert_financial': '💰 خبير مالي', 'expert_marketing': '📈 خبير تسويق', 'expert_operations': '⚙️ خبير عمليات',
        'placeholder': 'أدخل البيانات...\n\nمثال:\nالمبيعات: 50000\nالمصروفات: 30000',
        'btn_analyze': 'تحليل', 'results': 'النتائج', 'btn_copy': 'نسخ', 'btn_pdf': 'تصدير PDF', 'btn_new': 'تحليل جديد',
        'privacy_notice': '🔐 جميع البيانات تُعالج في RAM وتُحذف فوراً.', 'support': 'تحتاج مساعدة؟ axiomai873@gmail.com',
        'btn_logout': 'خروج', 'lang_btn': 'English', 'api_key': 'مفتاح Groq', 'api_help': 'احصل على المفتاح من console.groq.com'
    }
}
...
# ========================
def init():
    for k, v in [('logged_in', False), ('lang', 'ar'), ('name', ''), ('email', ''), ('age', 0), 
                 ('result', ''), ('input', ''), ('api_key', ''), ('denied', False)]:
...
def send_email(name, email, age):
    """Send notification to axiomai873@gmail.com via EmailJS API"""
    try:
...
def logout():
    for k in ['logged_in', 'name', 'email', 'age', 'result', 'input']:
        st.session_state[k] = '' if k in ['name', 'email', 'result', 'input'] else 0 if k == 'age' else False
    st.rerun()

def analyze(data, expert, api):
    prompts = {
        'financial': "You are a Financial Expert AI. Focus on ROI, costs, risks, and profits. Provide actionable insights.",
        'marketing': "You are a Marketing Expert AI. Focus on growth, branding, and customer trends. Provide strategic recommendations.",
        'operations': "You are an Operations Expert AI. Focus on efficiency, bottlenecks, and productivity. Provide practical improvements."
    }
    ...
    try:
        r = requests.post('https://api.groq.com/openai/v1/chat/completions',
            ...
            json={'model': 'llama-3.3-70b-versatile',
                  'messages': [{'role': 'system', 'content': prompts[expert]},
...
with c3:
    if st.button(t('lang_btn'), key='lang'): toggle_lang()
...
# ========================
    ...
    with c1:
        ...
        exp = st.selectbox(t('expert_label'), ['financial', 'marketing', 'operations'],
                          format_func=lambda x: t(f'expert_{x}'), key='exp')
    ...
    with c2:
        ...
        if st.session_state.result:
            ...
            with cb:
                st.download_button(t('btn_pdf'), gen_pdf(st.session_state.input, st.session_state.result),
                                 f"Report_{datetime.now().strftime('%Y%m%d')}.pdf", "application/pdf", key='pdf', use_container_width=True)
            if st.button(t('btn_new'), key='new', use_container_width=True):
                st.session_state.result = ''
                st.rerun()
        else:
            st.info("📊 " + ("النتائج هنا" if st.session_state.lang == 'ar' else "Results here"))
